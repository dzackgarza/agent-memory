...
def test_global_op_error_names_init_global_only_when_vault_missing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AGENT_MEMORY_VAULT", str(missing_vault))
    loose = unbound_dir(tmp_path)

    result = run_agent_memory_process(
        loose,
        "add",
        "--scope",
        "global",
