...
def test_unstarted_parent_with_started_child_flagged(tmp_path: Path) -> None:
    ...
    assert any(
        p.kind == "status-hierarchy"
        and p.card_id == "PLAN-ONE"
        and "started child 'PHASE-ONE'" in p.detail
        for p in problems
    )
...
def test_in_progress_parent_all_unstarted_children_flagged(tmp_path: Path) -> None:
    ...
    assert any(
        p.kind == "status-hierarchy"
        and p.card_id == "FEATURE-ONE"
        and "only unstarted children" in p.detail
        for p in problems
    )
    assert any(
        p.kind == "status-hierarchy"
        and p.card_id == "FEATURE-ONE"
        and "at least one started child" in p.detail
        for p in problems
    )
...
def test_complete_parent_with_incomplete_child_flagged(tmp_path: Path) -> None:
    ...
    assert any(
        p.kind == "status-hierarchy"
        and p.card_id == "FEATURE-ONE"
        and "while child 'PLAN-ONE'" in p.detail
        for p in problems
    )
...
def test_status_hierarchy_unstarted_parent_with_unstarted_child_silent(
    tmp_path: Path,
) -> None:
    config, models = models_and_config()
    root = tmp_path / "p" / "plans"
...
def test_status_hierarchy_complete_parent_with_complete_child_silent(
    tmp_path: Path,
) -> None:
    config, models = models_and_config()
    root = tmp_path / "p" / "plans"
...
def test_sibling_ordering_missing_dependson_flagged(tmp_path: Path) -> None:
    ...
    update_card(
        root, config, models, "FEATURE-ONE", {"plans": ["[[PLAN-A]]", "[[PLAN-B]]"]}
    )
    problems = validate_cards(load_card_records([root], config, models), config)
    assert any(
        p.kind == "sibling-ordering"
        and p.card_id == "PLAN-B"
        and "dependsOn 'PLAN-A'" in p.detail
        for p in problems
    )
...
def test_sibling_ordering_undeclared_children_flagged(tmp_path: Path) -> None:
    ...
    assert any(
        p.kind == "sibling-ordering"
        and p.card_id == "FEATURE-ONE"
        and "ordered 'plans' links" in p.detail
        for p in problems
    )
...
def test_sibling_ordering_omitted_child_flagged(tmp_path: Path) -> None:
    ...
    assert any(
        p.kind == "sibling-ordering"
        and p.card_id == "FEATURE-ONE"
        and "omits" in p.detail
        and "PLAN-B" in p.detail
        for p in problems
    )
...
def test_sibling_ordering_extra_declared_flagged(tmp_path: Path) -> None:
    ...
    update_card(
        ...
        models,
        "FEATURE-ONE",
        {"plans": ["[[PLAN-A]]", "[[PLAN-B]]", "[[TASK-GHOST]]"]},
    )
    ...
    assert any(
        p.kind == "sibling-ordering"
        and p.card_id == "FEATURE-ONE"
        and "non-child" in p.detail
        and "TASK-GHOST" in p.detail
        for p in problems
    )
...
def test_sibling_ordering_phase_to_task_level(tmp_path: Path) -> None:
    ...
    update_card(
        root, config, models, "PHASE-ONE", {"tasks": ["[[TASK-A]]", "[[TASK-B]]"]}
    )
    # TASK-B does not dependsOn TASK-A
    problems = validate_cards(load_card_records([root], config, models), config)
    assert any(
        p.kind == "sibling-ordering"
        and p.card_id == "TASK-B"
        and "dependsOn 'TASK-A'" in p.detail
        for p in problems
    )
...
def test_filesystem_hierarchy_misplaced_card_flagged(tmp_path: Path) -> None:
    ...
    records["PLAN-ONE"] = CardRecord(
        type_name=plan_record.type_name, path=wrong, metadata=plan_record.metadata
    )
    problems = validate_cards(records, config)
    assert any(
        p.kind == "filesystem-hierarchy" and p.card_id == "PLAN-ONE" for p in problems
    )
...
def test_tags_from_ancestry_mismatch_flagged(tmp_path: Path) -> None:
    ...
    problems = validate_cards(load_card_records([root], config, models), config)
    assert any(
        p.kind == "tags-from-ancestry" and p.card_id == "TASK-ONE" for p in problems
    )
...
def test_tags_from_ancestry_missing_flagged(tmp_path: Path) -> None:
    ...
    problems = validate_cards(load_card_records([root], config, models), config)
    assert any(
        p.kind == "tags-from-ancestry" and p.card_id == "PLAN-ONE" for p in problems
    )
...
def test_tags_from_ancestry_correct_tags_silent(tmp_path: Path) -> None:
    ...
    update_card(root, config, models, "PLAN-ONE", {"tags": ["FEATURE-ONE"]})
    update_card(
        root, config, models, "PHASE-ONE", {"tags": ["FEATURE-ONE", "PLAN-ONE"]}
    )
    update_card(
        root,
        config,
        models,
        "TASK-ONE",
        {"tags": ["FEATURE-ONE", "PLAN-ONE", "PHASE-ONE"]},
    )
    problems = validate_cards(load_card_records([root], config, models), config)
...
def test_tags_from_ancestry_no_ancestors_but_declares_tags_flagged(
    tmp_path: Path,
) -> None:
    config, models = models_and_config()
    root = tmp_path / "p" / "plans"
    ...
    problems = validate_cards(load_card_records([root], config, models), config)
    assert any(
        p.kind == "tags-from-ancestry"
        and p.card_id == "FEATURE-ONE"
        and "no tagged ancestors" in p.detail
        for p in problems
    )
...
def test_ancestor_chain_dedups_shared_ancestors() -> None:
    ...
    records: dict[str, CardRecord] = {
        "FEATURE-D": CardRecord(
            "feature", base / "features/FEATURE-D/FEATURE-D.md", {"id": "FEATURE-D"}
        ),
        "PLAN-A": CardRecord(
            "plan",
            base / "features/FEATURE-D/plans/PLAN-A/PLAN-A.md",
            {"id": "PLAN-A", "parents": ["[[FEATURE-D]]"]},
        ),
        "PLAN-B": CardRecord(
            "plan",
            base / "features/FEATURE-D/plans/PLAN-B/PLAN-B.md",
            {"id": "PLAN-B", "parents": ["[[FEATURE-D]]"]},
        ),
        ...
        "PHASE-D": CardRecord(
            "phase",
            base / "PHASE-D/PHASE-D.md",
            {"id": "PHASE-D", "parents": ["[[PLAN-A]]", "[[PLAN-B]]", "[[FEATURE-D]]"]},
        ),
    }
