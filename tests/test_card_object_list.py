...
CONFIG: dict[str, Any] = {
    ...
    "status_sets": {
        "standard": {"default": "unstarted", "options": ["unstarted", "complete"]}
    },
    "card_types": [
        {
            "name": "spec",
...
def test_object_list_validates_nested_items() -> None:
    inventory = {
        ...
        "sageNames": [],
        "projectNames": ["EmptyCategory"],
    }
    dumped = (
        spec_model()
        .model_validate({**base_card(), "inventories": [inventory]})
        .model_dump()
    )
...
def test_object_list_required_field_must_be_present() -> None:
    ...
    assert (
        model.model_validate({**base_card(), "inventories": [item]}).model_dump()[
            "inventories"
        ][0]["owner"]
        == "o"
    )
...
def test_config_rejects_schema_on_non_object_list_field() -> None:
    bad = deepcopy(CONFIG)
    bad["card_types"][0]["fields"].append(
        {"name": "extra", "type": "string", "schema": [{"name": "x", "type": "string"}]}
    )
    with pytest.raises(ValidationError):
        CardSystemConfig.model_validate(bad)
...
def test_config_rejects_nested_select_without_options() -> None:
    bad = deepcopy(CONFIG)
    bad["card_types"][0]["fields"][3]["schema"].append(
...
def test_shipped_spec_model_validates_real_frontmatter_with_inventories() -> None:
    ...
    card = {
        ...
        "acceptanceCriteria": [
            "Source paths are reviewed.",
            "Every admitted row states evidence.",
        ],
        "complexity": 80,
    ...
    assert validated["constructorNameInventories"][0][
