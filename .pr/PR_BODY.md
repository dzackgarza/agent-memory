...
This is a planning/claim scaffold so this work unit is picked up as one coherent, review-sized PR instead of fragmented one-issue-per-PR. It carries no implementation yet; the implementer pushes commits here and switches `Refs` to `Closes` as each obligation lands with proof.
...
- **Proof obligations claimed:** Atomicity: forced failure mid-`add` leaves no partial file and a clean vault.
  Graceful handling: delete/plan add/init project handle real inputs without uncaught AssertionError/FileExistsError.
...
Each claimed issue lands with a committed red reproducer that fails on current behavior and passes after the fix, verified under `just test-ci`.
