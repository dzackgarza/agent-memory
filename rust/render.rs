// Vendored from iwe-org/iwe crates/iwe/src/render.rs (rev 71d5fb3, v0.3.2).
//
// `iwe retrieve -k <key>` prints the markdown produced by the CLI's RetrieveRenderer,
// which lives in the iwe binary crate (not the liwe library) and therefore cannot be
// imported. The rendering is reproduced here verbatim against liwe's public API so the
// binding emits byte-identical retrieve output to the upstream binary. All helpers below
// depend only on public `liwe` items (GraphContext, Projector, TreeIter, writer, Key).
