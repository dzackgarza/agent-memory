...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| IOB-001 `vault init`      | TEST-001 runs `iwe2 vault init <vault>` and checks Git repository/index status, the IWE config, and root/global graph indexes. Commit `af6db4b` added the red graph-link proof; commit `e0a57a8` made it green.                 | A CLI that creates directories but omits Git versioning, staging, or IWE graph inclusion links fails verification.                          |
...
| IOB-003 note creation     | TEST-003 creates project and global notes through the CLI and inspects their scoped paths, frontmatter, and staged Git status.                                                                                                  | A note writer that ignores scope, type directories, frontmatter, or vault staging fails.                                                    |
