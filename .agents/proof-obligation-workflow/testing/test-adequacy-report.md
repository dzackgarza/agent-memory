...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IOB-001 `vault init`      | TEST-001 runs `iwe2 vault init <vault>` and checks Git repository/index status, the IWE config, `zk` notebook files, ignored `zk` database, and root/global graph indexes. Commit `af6db4b` added the red graph-link proof; commit `e0a57a8` made it green. | A CLI that creates directories but omits Git versioning, staging, IWE graph inclusion links, or `zk` notebook setup fails verification.                  |
...
| IOB-003 note creation     | TEST-003 creates project and global notes through the CLI and inspects their scoped paths, frontmatter, and staged Git status; TEST-011 proves wrapper-created notes are visible through `zk` indexed search.                                               | A note writer that ignores scope, type directories, frontmatter, vault staging, or index refresh fails.                                                  |
...
| Missing `iwe`, `rg`, `git`, `npx`, Probe, or `zk` binaries     | Deferred  | These are hard dependencies for this bespoke system. Setup failure should be loud, not tested through optional-dependency branches. |
