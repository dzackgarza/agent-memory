...
implementations for IOB-001 through IOB-011.
...
real Markdown files, real IWE commands, real IWE `find`, real `rg` search, real
Probe subprocesses through `npx`, and a real `zk` release binary. The suite does not use mocks, source-text
...
unscoped `zk` indexed search, and hollow doctor output.
