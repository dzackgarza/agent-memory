...
real Markdown files, real IWE commands, and real `rg` search. The suite does not use
...
| IOB-002 `project init`     | TEST-002 creates a real Git repo with a remote, initializes a project, reads `.agent-memory.toml`, and checks the project IWE tree.                                                | A hard-coded project ID, wrong vault pointer, or missing project graph children fails.               |
...
| IOB-004 scoped body search | TEST-003 writes distinct dynamic body strings and runs `search --scope project`, `global`, and `both`.                                                                             | A search command rooted at the wrong subtree, returning placeholder output, or ignoring scope fails. |
