...
- Do not build a local search database or vector search layer. `zk` is used only as the delegated rebuildable indexed Markdown search engine; Probe is implemented only as an explicit ranked contextual search command; IWE `find` proves scoped title/key graph search and `rg` proves scoped exact body search.
