...
| Automatic Git staging or commits for the vault | Transcript says promotion may optionally commit or stage changes.                     | The wrapper initializes the vault Git repository, writes durable files, and leaves staging/commit policy to the caller. | Not required by any MVP IOB.             |
