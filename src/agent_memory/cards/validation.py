    ...
    for root in plans_roots:
        ...
        for path in sorted(root.rglob("*.md")):
            ...
            records[card_id] = CardRecord(
                type_name=card_type.name, path=path, metadata=metadata
            )
    return records
...
def reference_field_names(config: CardSystemConfig) -> dict[str, list[str]]:
    return {
        card_type.name: [
            ...
            if field.type in ("wikilink", "wikilink_list")
        ]
            ...
            if target not in records:
                problems.append(
                    Problem(
                        "reference",
                        card_id,
                        f"{field_name} references missing card: {target}",
                    )
                )
    return problems
...
def containment_problems(
    ...
    for parent_id in wikilink_ids(record.metadata.get("parents") or []):
        if (
            ...
            and records[parent_id].type_name not in card_type.parents
        ):
            problems.append(
                Problem(
                    "containment",
                    card_id,
                    f"parent {parent_id} is a {records[parent_id].type_name}, not in allowed {card_type.parents}",
                )
            )
    return problems
...
def depends_targets(record: CardRecord, records: dict[str, CardRecord]) -> list[str]:
    return [
        target
        for target in wikilink_ids(record.metadata.get("dependsOn") or [])
...
def card_status(record: CardRecord) -> str:
    ...
    assert isinstance(status, str) and status, (
        f"card status must be a non-empty string: {record.path}"
    )
    return status
...
def _unstarted_parent_problems(
    parent_id: str, parent_status: str, statuses: dict[str, str], roles: StatusRoles
) -> list[Problem]:
    ...
    return [
        Problem(
            "status-hierarchy",
            parent_id,
            f"status '{parent_status}' cannot contain started child '{child_id}' status '{statuses[child_id]}'",
        )
    ]
    ...
    if not started_children:
        problems.append(
            Problem(
                "status-hierarchy",
                parent_id,
                f"status '{parent_status}' requires at least one started child",
            )
        )
    return problems


def _complete_parent_problems(
    parent_id: str, parent_status: str, statuses: dict[str, str], roles: StatusRoles
) -> list[Problem]:
    ...
    incomplete_children = [
        child_id
        for child_id, status in statuses.items()
        if status not in roles.complete
    ]
    ...
    return [
        Problem(
            "status-hierarchy",
            parent_id,
            f"status '{parent_status}' cannot be complete while child '{child_id}' is '{statuses[child_id]}'",
        )
    ]


def status_hierarchy_problems(
    records: dict[str, CardRecord], config: CardSystemConfig
) -> list[Problem]:
    ...
    for parent_id, child_ids in children_by_parent(records).items():
        ...
        problems.extend(
            _unstarted_parent_problems(parent_id, parent_status, statuses, roles)
        )
        problems.extend(
            _started_parent_problems(parent_id, parent_status, statuses, roles)
        )
        problems.extend(
            _complete_parent_problems(parent_id, parent_status, statuses, roles)
        )
    return problems
...
def _mismatch_problem(
    parent_id: str, child_type: str, field_name: str, label: str, ids: list[str]
) -> list[Problem]:
    if not ids:
        return []
    return [
        Problem(
            "sibling-ordering",
            parent_id,
            f"'{field_name}' {label} {child_type} ids: {', '.join(ids)}",
        )
    ]
...
def _reconcile_declared_order(
    ...
    problems = _mismatch_problem(
        parent_id, child_type, field_name, "omits", missing
    ) + _mismatch_problem(
        parent_id, child_type, field_name, "references non-child", extras
    )
        ...
        return OrderedChildren(
            ...
            [
                Problem(
                    "sibling-ordering",
                    parent_id,
                    f"parent with multiple {child_type} children must declare ordered '{field_name}' links",
                )
            ],
        )
    return _reconcile_declared_order(
        parent_id, child_type, field_name, declared, children
    )


def _dependson_order_problems(
    sequence: list[str], records: dict[str, CardRecord]
) -> list[Problem]:
    return [
        Problem(
            "sibling-ordering",
            child_id,
            f"sibling order requires dependsOn '{previous_id}'",
        )
...
def ordered_child_fields(config: CardSystemConfig) -> dict[str, tuple[str, str]]:
    return {
        parent_type.name: (child_type, field_name)
        for parent_type in config.card_types
        for child_type, field_name in parent_type.ordered_children.items()
    }


def sibling_ordering_problems(
    records: dict[str, CardRecord], config: CardSystemConfig
) -> list[Problem]:
    ...
    for parent_id in sorted(records):
        ...
        typed = [
            ...
            if records[child_id].type_name == child_type
        ]
...
def plans_root_for(
    ...
    if card_type.container and card_type.container != config.root:
        assert card_type.container in parts, (
            f"root card not under its configured container {card_type.container}: {record.path}"
        )
        return Path(*parts[: parts.index(card_type.container)])
    assert config.root in parts, (
        f"root card not under configured root {config.root}: {record.path}"
    )
    return Path(*parts[: parts.index(config.root) + 1])


def _filesystem_problem(
    card_id: str,
    record: CardRecord,
    card_type: CardTypeSpec,
    records: dict[str, CardRecord],
    ...
    expected = card_file_path(
        plans_root_for(card_id, record, records, config), card_type, card_id, parent_id
    ).resolve()
    ...
    return Problem(
        ...
        card_id,
        f"expected path {expected}, found {record.path.resolve()}",
    )


def filesystem_hierarchy_problems(
    records: dict[str, CardRecord], config: CardSystemConfig
) -> list[Problem]:
    ...
    problems = [
        _filesystem_problem(
            card_id,
            records[card_id],
            by_type[records[card_id].type_name],
            records,
            config,
        )
...
def ancestor_chain(
    card_id: str, records: dict[str, CardRecord], active: frozenset[str]
) -> list[str]:
...
def tags_from_ancestry_problems(
    records: dict[str, CardRecord], config: CardSystemConfig
) -> list[Problem]:
    # Port of the derive-tags logic: a card's tags must equal the chain of its ancestor ids
    # whose type opts into tagged ancestry.
    tagged_types = {
        card_type.name for card_type in config.card_types if card_type.tagged_ancestor
    }
    problems: list[Problem] = []
    for card_id in sorted(records):
        derived = [
            ancestor_id
            for ancestor_id in ancestor_chain(card_id, records, frozenset())
            if records[ancestor_id].type_name in tagged_types
        ]
        ...
        if derived:
            if tags != derived:
                problems.append(
                    Problem(
                        "tags-from-ancestry",
                        card_id,
                        f"tags must equal ancestor chain {derived}, found {tags}",
                    )
                )
            ...
            problems.append(
                Problem(
                    "tags-from-ancestry",
                    card_id,
                    f"card has no tagged ancestors but declares tags {tags}",
                )
            )
    return problems
...
def validate_cards(
    ...
    for card_id, record in records.items():
        card_type = by_type[record.type_name]
        problems.extend(
            reference_problems(card_id, record, ref_fields[record.type_name], records)
        )
